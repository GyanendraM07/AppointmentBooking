...
import { createBranchApi, updateBranch } from '../../../api/Apicall';
...
function TabPanel(props) {
    ...
    const [validationErrors, setValidationErrors] = useState({ name: false, shortName: false, isActive: false });
    ...
    const branchDetailsChange = (field, value) => {
    ...
    const createBranch = async () => {
        ...
        if (isValid) {
            try {
                ...
                const resp = await createBranchApi(modifiedFormData, token);
                debugger;
                if (resp.status) {
    ...
    const tableData = filterBranchList.map((branch, index) => {
        const isActiveText = branch.isActive == 1 ? 'Active' : 'InActive';
        return createData(
            branch._id,
            branch.createdDate,
            branch.name,
    ...
    const onEditClick = (branchId) => {
        const branch = filterBranchList.find(branch => branch._id === branchId);
        const data = {
            Name: { value: branch.name, label: 'Name', editable: false, type: 'text', branchId: branchId },
            ShortName: { value: branch.shortName, label: 'ShortName', editable: false, type: 'text', branchId: branchId },
            Status: {
    ...
    const saveEditedUser = async (editBranchData) => {
        const updatedBranch = {
            name: editBranchData.Name.value,
            shortName: editBranchData.ShortName.value,
            isActive: editBranchData.Status.value,
            updatedBy: store.getState().user.user.userId,
            _id: editBranchData.Name.branchId
        }
        const token = sessionStorage.getItem('token');
        const resp = await updateBranch(updatedBranch, token);
        if (resp.status) {
            handleClick(resp.message, 'success');
            dispatch({ type: 'SET_COMPANY_LIST', payload: { companyList: resp.companyList } });
            setShowialog(false);
    ...
    return (
        <Typography
            ...
            {...other}
            sx={{ width: '950px' }}// make changes on every module 
        >
            {value === index && (
                <Box sx={{ p: 3 }}>
                    {showAlert && (
                    ...
                    {index === 1 && (
                        <Grid container spacing={2} sx={{ maxWidth: 600, margin: 'auto' }}>
                            <Grid item xs={12} sm={6}>
                                <TextField fullWidth id="standard-basic-1" label="Name" variant="standard" onChange={(e) => branchDetailsChange('name', e.target.value)} required error={validationErrors.name} value={formData.name} />
                            </Grid>
                            <Grid item xs={12} sm={6}>
                                <TextField fullWidth id="standard-basic-2" label="Short Name" variant="standard" onChange={(e) => branchDetailsChange('shortName', e.target.value)} required error={validationErrors.shortName} value={formData.shortName} />
                            </Grid>
                            <Grid item xs={12} sm={6}>
                                <FormControl variant="standard" sx={{ m: 1, width: '100%' }}>
                                    ...
                                    <Select
                                        ...
                                        onChange={(e) => branchDetailsChange('isActive', e.target.value)}
                                        label="Is Active"
                                        required
    ...
    return (
        ...
        >
            <AppBar position="static" color="default">
                ...
                >
                    <Tab label="Branch List" {...a11yProps(0)} sx={{ whiteSpace: 'nowrap' }} />
                    <Tab label="Create Branch" {...a11yProps(1)} sx={{ whiteSpace: 'nowrap' }} />
    ...
    return (
        <>
            <Grid item xs={12} sm={6} sx={{ display: 'flex', justifyContent: 'center' }}>
                <Box sx={{ minWidth: '26cm', position: 'fixed', zIndex: 1000 }}>
                    <Stack spacing={2}>
                        <Item>Branch Config</Item>
                    </Stack>
                </Box>
            </Grid>
            <Grid item xs={12} sm={6} sx={{ display: 'flex', justifyContent: 'center', }}>
