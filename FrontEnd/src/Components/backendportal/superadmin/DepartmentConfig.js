...
import { createDepartmentApi, updateDepartment } from '../../../api/Apicall';

function TabPanel(props) {
    const { children, value, index, handleClick, ...other } = props;
    const dispatch = useDispatch();
    const [formData, setFormData] = useState({ name: '', shortName: '', isActive: '', branchId: '' });
    const [validationErrors, setValidationErrors] = useState({ name: false, shortName: false, isActive: false, branchId: false }); // used for createUser Validation
    ...
    const createDepartment = async () => {
        ...
        if (isValid) {
            try {
                ...
                debugger;
                const resp = await createDepartmentApi(modifiedFormData, token);
                debugger;
                if (resp.status) {
    ...
    const deptDetailsChange = (field, value) => {
    ...
    const headCells = [
        ...
        { id: 'branchName', numeric: false, disablePadding: true, label: 'BranchName' },
    ...
    const tableData = filterDeptList.map((dept, index) => {
        const isActiveText = dept.isActive == 1 ? 'Active' : 'InActive';
        const branchN = getBranchName(dept.branchId);
        debugger;
        return createData(
            dept._id,
            dept.createdDate,
    ...
    function createData(id, createdDate, name, shortName, branchName, isActive) {
        return {
            id,
            createdDate,
            name,
    ...
    const onEditClick = (deptId) => {
        const dept = filterDeptList.find(dept => dept._id === deptId);
        const branchN = getBranchName(dept.branchId);
        const data = {
            Name: { value: dept.name, label: 'Name', editable: false, type: 'text', deptId: deptId },
            ShortName: { value: dept.shortName, label: 'ShortName', editable: false, type: 'text', deptId: deptId },
            BranchName: { value: branchN, label: 'BranchName', editable: true, type: 'text', deptId: deptId },
            Status: {
                value: dept.isActive,
                label: 'Status',
                editable: true,
    ...
    const saveEditedUser = async (editDeptData) => {
        const updatedDept = {
            name: editDeptData.Name.value,
            shortName: editDeptData.ShortName.value,
            isActive: editDeptData.Status.value,
            updatedBy: store.getState().user.user.userId,
            _id: editDeptData.Name.deptId
        }
        const token = sessionStorage.getItem('token');
        const resp = await updateDepartment(updatedDept, token);
        if (resp.status) {
            handleClick(resp.message, 'success');
            dispatch({ type: 'SET_COMPANY_LIST', payload: { companyList: resp.companyList } });
            setShowialog(false);
                    ...
                    {index === 1 && (
                        <Grid container spacing={2} sx={{ maxWidth: 600, margin: 'auto' }}>
                            <Grid item xs={12} sm={6}>
                                <TextField fullWidth id="standard-basic-1" label="Name" variant="standard" onChange={(e) => deptDetailsChange('name', e.target.value)} required error={validationErrors.name} value={formData.name} />
                            </Grid>
                            <Grid item xs={12} sm={6}>
                                <TextField fullWidth id="standard-basic-2" label="Short Name" variant="standard" onChange={(e) => deptDetailsChange('shortName', e.target.value)} required error={validationErrors.shortName} value={formData.shortName} />
                            </Grid>
                            <Grid item xs={12} sm={6} sx={{ display: 'flex', justifyContent: 'center' }}>
                                <FormControl variant="standard" sx={{ m: 1, width: '100%' }}>
                                    ...
                                    <Select
                                        ...
                                        onChange={(e) => deptDetailsChange('branchId', e.target.value)}
                                        label="SelectBranch"
                            ...
                            <Grid item xs={12} sm={6}>
                                <FormControl variant="standard" sx={{ m: 1, width: '100%' }}>
                                    ...
                                    <Select
                                        ...
                                        onChange={(e) => deptDetailsChange('isActive', e.target.value)}
                                        label="Is Active"
                                        required
                            ...
                            <Grid item xs={12} sm={6}>
                                <FormControl variant="standard" sx={{ m: 1, width: '100%' }}>
                                    <TextField
                                        ...
                                        onChange={(e) => deptDetailsChange('imageURL', e.target.value)}
                                        required
    ...
    return (
        ...
        >
            <AppBar position="static" color="default">
                ...
                >
                    <Tab label="Department List" {...a11yProps(0)} sx={{ whiteSpace: 'nowrap' }} />
                    <Tab label="Create Department" {...a11yProps(1)} sx={{ whiteSpace: 'nowrap' }} />
